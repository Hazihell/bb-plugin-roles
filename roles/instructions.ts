...
function buildCastSection(roles: Role[]): string {
  const lines = roles.map(
    (role) =>
      `- **${role.id}** (${role.candidates[0]!.reasoningLevel}) — ${truncate(role.description, DESCRIPTION_MAX)}${role.brief === undefined ? "" : ` Brief: ${role.brief}`}`,
  );
  return [
    "## Cast",
    ...lines,
...
function buildRoleSection(role: Role, parentThreadId: string | null): string | null {
  ...
  const instruction = truncate(
    role.instruction,
    Math.max(0, MAX_LENGTH - prefix.length - CHILD_LEAF_CONTRACT.length - coordinator.length - SECTION_SEPARATOR.length),
  );
  return `${prefix}${instruction}${SECTION_SEPARATOR}${CHILD_LEAF_CONTRACT}${coordinator}`;
}
