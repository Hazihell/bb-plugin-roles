...
// surfaces: the Example todos page (app.tsx, over RPC), the `bb roles` CLI
...
export default async function plugin(bb: BbPluginApi) {
  ...
  // `bb plugin config roles`. Add `secret: true` for values like API keys.
  ...
  // The `bb roles` command: what agents (and you) use from a shell. Parsing
  ...
  const usage = [
    "Usage:",
    "  bb roles list [--json]",
    "  bb roles add <title> [--json]",
    "  bb roles done <todo-id> [--json]",
    "  bb roles undo <todo-id> [--json]",
    "  bb roles remove <todo-id> [--json]",
  ].join("\n");
  ...
  bb.cli.register({
    name: "roles",
    summary: "Manage the Roles plugin's example todo list",
    commands: [
      { name: "list", summary: "List todos", usage: "bb roles list [--json]" },
      {
        name: "add",
        summary: "Add a todo",
        usage: "bb roles add <title> [--json]",
      },
      {
        name: "done",
        summary: "Mark a todo done",
        usage: "bb roles done <todo-id> [--json]",
      },
      {
        name: "undo",
        summary: "Mark a todo not done",
        usage: "bb roles undo <todo-id> [--json]",
      },
      {
        name: "remove",
        summary: "Remove a todo",
        usage: "bb roles remove <todo-id> [--json]",
      },
    ],
    async run(argv) {
      ...
      const notFound = (missingId: string) => ({
        exitCode: 1,
        stderr: `No todo with id ${missingId}. Run "bb roles list" to see ids.`,
      });
      const todoId = args[0];
      switch (command) {
        case undefined:
