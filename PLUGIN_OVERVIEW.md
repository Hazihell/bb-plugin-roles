...
- An **Example todos** page in the left sidebar that adds, completes, and
  removes todos.
- A `bb roles` command that does the same from a terminal.
...
The bundled skill tells an agent to read the list with `bb roles list`, add
one todo at a time with `bb roles add`, and close finished work with
`bb roles done`.
